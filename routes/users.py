...
def list_users(request: Request):
    users_from_db = list(request.app.database["users"].find(limit=100))
    users = [User(**user) for user in users_from_db]  # Assuming User model has attributes matching the keys in the MongoDB documents
    return users
...
def delete_user(id: str, request: Request, response: Response):
    try:
        user_id = ObjectId(id)  # Convert id to ObjectId
